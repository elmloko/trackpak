...
class ApiController extends Controller
{
    public function busquedaRRApi(Request $request)
    {
        ...
        $correo  = 'Correos';
        $clave   = 'AGBClp2020';
        $baseUrl = 'http://172.65.10.37';

        // Datos locales
        $packages = Package::withTrashed()
            ->where('CODIGO', $codigo)->take(5)->get();

        $eventosLocales = Event::where('codigo', $codigo)
            ->where('action', '!=', 'ESTADO')
        ...
        try {
            ...
            if (!$token) {
                return response()->json([
                    'message' => 'Auth externo sin token',
                ], 502);
            }
        } catch (\Throwable $e) {
            return response()->json([
                'message' => 'Error conectando a auth externo',
                'error'   => $e->getMessage(),
            ], 502);
        }
        ...
        try {
            $resp = Http::timeout(15)
                ...
                ->acceptJson()
                ->withToken($token)
                ->post('/api/O_MAIL_OBJECTS/buscar', ['id' => $codigo]);

            if ($resp->failed()) {
                return response()->json([
                    'message' => 'Búsqueda externa falló',
                    'status'  => $resp->status(),
                    'body'    => $resp->body(),
                ], 502);
            }
            ...
            foreach ($results as &$r) {
                $r['eventType'] = $this->mapEventType($r['eventType'] ?? '');
            }
            usort(
                $results,
                fn($a, $b) =>
                strtotime($b['eventDate'] ?? '1970-01-01') <=> strtotime($a['eventDate'] ?? '1970-01-01')
            );
            $eventosExternos = $results;
        } catch (\Throwable $e) {
            return response()->json([
                'message' => 'Error consultando servicio externo',
                'error'   => $e->getMessage(),
            ], 502);
        }
        ...
        return response()->json([
            'codigo'           => $codigo,
            'packages'         => $packages,
            'eventos_locales'  => $eventosLocales,
