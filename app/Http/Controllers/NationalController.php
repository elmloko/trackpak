...
use App\Exports\DespachoadmisionExport;
use App\Exports\NacionalexpedisionExport;
use App\Models\User;
...
class NationalController extends Controller
{
    ...
    public function nacionalexpedision()
    {
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($peso >= 0.001 && $peso <= 0.020) {
                $precio = $this->calcularPrecio($provincia, $peso, [
                    'LOCAL 1' => 3,
                    'LOCAL 2' => 4,
                    'LOCAL 3' => 5,
                    'LOCAL 4' => 6,
            ...
            // Comprobar si el campo TELEFONODESTINATARIO tiene más de un carácter
            if (strlen($request->input('TELEFONODESTINATARIO')) > 1) {
                $importe += 2;
            }
    ...
    private function calcularPrecio($provincia, $peso, $importe)
    {
        // Verificar si la provincia está en el arreglo de precios
        if (array_key_exists($provincia, $importe)) {
            return $importe[$provincia];
        }

        // Agregar lógica para otros casos o lanzar una excepción si es necesario
        return 0;
    }
    ...
    public function despachoadmisionexcel(Request $request)
    {
        ...
        return Excel::download(new DespachoadmisionExport($fechaInicio, $fechaFin), 'Kardex Ventanilla.xlsx');
    }
    public function nacionalexpedisionexcel(Request $request)
    {
        $fechaInicio = $request->input('fecha_inicio');
        $fechaFin = $request->input('fecha_fin');
        return Excel::download(new NacionalexpedisionExport($fechaInicio, $fechaFin), 'Kardex Cartero.xlsx');
    }
